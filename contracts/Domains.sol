...
// 우리가 가져온 컨트랙트를 상속하는데 이를 통해 상속한 컨트랙트의 함수들을 사용할 수 있다.
contract Domains is ERC721URIStorage {
	// OpenZeppelin을 통해 tokenId를 추적할 수 있다
	using Counters for Counters.Counter;
	Counters.Counter private _tokenIds;

	string public tld;

	// SVG 형태로 NFT 이미지를 온체인 상에 저장할 것이다.
	...
	constructor(string memory _tld) payable ERC721("BoomLabs Name Service", "BOOM") {
		tld = _tld;
    ...
    function price(string calldata name) public pure returns(uint) {
        ...
        if (len == 3) {
        return 5 * 10**14;
        } else if (len == 4) { // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals). 
        return 3 * 10**14;
        } else {
        return 1 * 10**14;
        }
	...
	function register(string calldata name) public payable {
		...
		// 전달된 도메인 이름과 tld를 연결합니다
		string memory _name = string(abi.encodePacked(name, ".", tld));
		
		// 만들어진 이름을 가지고 NFT를 만들기 위해 SVG 이미지를 생성합니다
		...
		// NFT를 위한 JSON 메타데이터를 생성합니다. 이는 string 값을 합쳐서 base64 형태로 인코딩합니다
